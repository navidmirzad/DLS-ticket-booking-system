...
export const createTicket = async (eventId, userId, email, ticketsBought) => {
    ...
    for (const item of ticketsBought) {
        ...
        switch (ticketTypeIdentifier.toLowerCase()) {
            ...
            default:
                ...
                console.warn(`Unknown ticket type identifier: ${ticketTypeIdentifier}. Defaulting to STANDARD.`);
                resolvedTicketPrice = 50; // Default price
                resolvedTicketTypeString = "STANDARD"; // Default type from enum
        }

        if (!resolvedTicketTypeString) {
            throw new Error(`Invalid ticket type identifier received: ${ticketTypeIdentifier}`);
        }
    ...
    const eventInfo = {
        ...
        description: event.event_description?.event_description || event.description
    };
